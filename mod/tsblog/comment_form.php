...
class mod_tsblog_comment_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($moderated) {
            $mform->addElement('static', '', '',
                    get_string('moderated_info', 'tsblog', $CFG->wwwroot .
                        '/mod/tsblog/bloglogin.php?returnurl=editcomment.php?blog=' .
                        $blogid . '%26post=' . $postid));

            $mform->addElement('text', 'authorname',
                    get_string('moderated_authorname', 'tsblog'), 'size="48"');
        ...
        $mform->addElement($messagetype, 'messagecomment', get_string('comment', 'tsblog'),
        ...
        if ($moderated && !$confirmed) {
            $mform->addElement('static', '', '',
                    get_string('moderated_confirminfo', 'tsblog'));
            $mform->addElement('text', 'confirm', get_string('moderated_confirm', 'tsblog'));
        }

        if ($edit) {
            $submitstring = get_string('savechanges');
        } else {
            $submitstring = get_string('addcomment', 'tsblog');
        }
    ...
    public function validation($data, $files) {
        ...
        if ($moderated && !$confirmed && (empty($data['confirm']) ||
                $data['confirm'] !== get_string('moderated_confirmvalue', 'tsblog'))) {
            $errors['confirm'] = get_string('error_noconfirm', 'tsblog');
        }
        return $errors;
    }
}
