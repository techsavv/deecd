...
$url = new moodle_url('/mod/tsblog/feed.php', array('format'=>$format, 'blog'=>$blogid,
...
if (empty($CFG->enablerssfeeds)) {
    print_error('feedsnotenabled', 'tsblog');
}
if ($format != 'atom' && $format != 'rss') {
    print_error('invalidformat', 'tsblog');
}
...
if (isset($bloginstancesid) && $bloginstancesid!='all') {
    $bloginstance = $DB->get_record('tsblog_instances', array('id'=>$bloginstancesid));
    $blog         = $DB->get_record('tsblog', array('id'=>$bloginstance->tsblogid));
} else if ($blogid) {
    $blog = $DB->get_record('tsblog', array('id'=>$blogid));

} else if ($postid) {
    $post         = $DB->get_record('tsblog_posts', array('id'=>$postid));
    $bloginstance = $DB->get_record('tsblog_instances', array('id'=>$post->tsbloginstancesid));
    $blog         = $DB->get_record('tsblog', array('id'=>$bloginstance->tsblogid));
}
if (!isset($blog->id) || !$cm = get_coursemodule_from_instance('tsblog', $blog->id)) {
...
if ($blog->global) {
    if ($bloginstancesid == 'all') {
        $url = $CFG->wwwroot . '/mod/tsblog/allposts.php';
    } else {
        $url = $CFG->wwwroot . '/mod/tsblog/view.php?user=' .
            $bloginstance->userid;
    }
    $groupmode = 0;
} else {
    $url = $CFG->wwwroot . '/mod/tsblog/view.php?id=' . $cm->id .
...
if (check_browser_version('MSIE', 0) || check_browser_version('Firefox', 0)) {
    if (!check_browser_version('MSIE', '7') && !check_browser_version('Firefox', '2')) {
        ...
        print_error('unsupportedbrowser', 'tsblog', $url);
    }
}
...
header('Pragma: ');
header('Expires: ');
if ($mtime = tsblog_feed_last_changed($blogid, $bloginstancesid, $postid, $comments)) {
...
if ($full) {
    ...
    if ($full == md5($accesstoken.$viewer.TS_BLOG_VISIBILITY_COURSEUSER.'v2') && $user =
            $DB->get_record('user', array('id'=>$viewer))) {
        $allowedvisibility = TS_BLOG_VISIBILITY_COURSEUSER;
    } else if ($full == md5($accesstoken.$viewer.TS_BLOG_VISIBILITY_COURSEUSER) && $user =
            $DB->get_record('user', array('id'=>$viewer))) {
        // This is the old token. Ooops. We know that at least users were
        // logged in, so they get that version...
        $allowedvisibility = TS_BLOG_VISIBILITY_LOGGEDINUSER;
        if (!$blog->global) {
    ...
    if ($loggedin == md5($accesstoken.$viewer.TS_BLOG_VISIBILITY_LOGGEDINUSER) && $user =
            $DB->get_record('user', array('id'=>$viewer))) {
        $allowedvisibility = TS_BLOG_VISIBILITY_LOGGEDINUSER;
    } else {
        print_error('nopermissiontoshow');
    ...
    $allowedvisibility = TS_BLOG_VISIBILITY_PUBLIC;
    $user = $USER;
}

// Check individual
if ($blog->individual) {
    if (!tsblog_individual_has_permissions($cm, $blog, $groupid, $individualid, $user->id)) {
         print_error('nopermissiontoshow');
    }
}
...
if ($comments) {
    $feeddata = tsblog_get_feed_comments($blogid, $bloginstancesid, $postid, $user,
            $allowedvisibility, $groupid, $cm, $blog, $individualid);
    $feedname = strip_tags($blog->name) . ': ' . get_string('commentsfeed', 'tsblog');
    $feedsummary='';
} else {
    $feeddata = tsblog_get_feed_posts($blogid,
